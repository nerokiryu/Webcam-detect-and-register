...
const stopWebcamButton = document.getElementById("webcamStopButton");
...
// Disable the live webcam view and stop classification.
function stopCam(event) {
  // Hide the button once clicked.
  event.target.classList.add("removed");
  enableWebcamButton.classList.remove("removed");

  // Deactivate the webcam stream.
  ...
  model.detect(video).then(function (predictions) {
    ...
    for (let n = 0; n < predictions.length; n++) {
      ...
      if (predictions[n].score > 0.66) {
        ...
        children.push(p);
        if (!register && predictions[n].class == "person") {
          register = true;
          registerVideo();
          window.setTimeout(stopRecord, 10000);
        }
      }
    }
